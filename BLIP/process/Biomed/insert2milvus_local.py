...
# print("clip_tokenizer:", tokenizer)  #clip_tokenizer: <open_clip.clip_tokenizer.HFTokenizer object at 0x0000024F57CEB2B0>
...
# 读取 Excel 文件
# excel_file = 'DMSA图像文本描述数据_翻译_new.xlsx'  # 请替换为你实际的文件路径
excel_file = 'translated_recheck.xlsx'  # 请替换为你实际的文件路径
# excel_file = 'translated_output.xlsx'  # 请替换为你实际的文件路径
df = pd.read_excel(excel_file)

# 获取图像描述和图像名称列的数据
descriptions = df['Translated_Description'].tolist()
# descriptions = df['Image_description_en'].tolist()
image_names = df['Image'].tolist()

# 设置图像文件所在的路径
image_folder_path = 'D:/maskrcnn/datasets/新数据（首诊、复诊）/复诊_重命名'  # 改为正确的相对路径
# image_folder_path = 'D:/maskrcnn/datasets/新数据（首诊、复诊）/首诊'  # 改为正确的相对路径
# image_folder_path = 'D:/maskrcnn/datasets/images'  # 改为正确的相对路径
...
# client.load_collection("wjy_text_embedding_collection")

res = client.query(
        collection_name="wjy_image_embedding_collection",
        output_fields=["count(*)"]
    )
data_count = res[0]['count(*)']
print(data_count)
...
for idx, (description, image_name) in enumerate(zip(descriptions, image_names)):
    ...
    # 存储文本数据
    text_data.append({"id": data_count, "vector": text_vector, "text": description})
    ...
    try:
        ...
        new_image_name = 'recheck_' + image_name

        # 确保在转换之前移除梯度信息
        image_vectors = image_result[0].detach().cpu().numpy().tolist()  # 先断开梯度，再转换为列表

        # 存储图像数据
        image_data.append({"id": data_count, "vector": image_vectors, "text": new_image_name})
